...
func TestCheckPrintsPassingNodeGates(t *testing.T) {
	...
	assert.Equal(t, `PASS: node worker-1 exists
PASS: node worker-1 is ready
PASS: node worker-1 is active
SAFE: node worker-1 passed checks
`, stdout.String())
}
...
func TestCheckPrintsBlockersBeforePasses(t *testing.T) {
	...
	assert.Equal(t, `BLOCKER: node worker-1 state is down, expected ready
PASS: node worker-1 exists
PASS: node worker-1 is active
UNSAFE: node worker-1 failed checks
`, stdout.String())
}
...
func TestCheckJSONOutput(t *testing.T) {
  ...
  "findings": [
    ...
    },
    {
      "gate": "target_active",
      "level": "pass",
      "message": "node worker-1 is active"
    }
  ]
}
`, stdout.String())
